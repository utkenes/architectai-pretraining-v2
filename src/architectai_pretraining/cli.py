...
from architectai_pretraining.corpus_v2 import CorpusV2Pipeline, load_audit
...
def main() -> None:
    ...
    for action, help_text in (
        ...
        action_parser = corpus_actions.add_parser(action, help=help_text)
        action_parser.add_argument("--config", default="configs/corpus_v2.yaml")
        action_parser.add_argument("--corpus-root", default=None, help="External read-only corpus root (or ARCHITECT_DATA_DIR)")
        action_parser.add_argument("--output-dir", default=f"data/corpus_v2/{action}")
        if action in {"preview", "freeze"}:
            action_parser.add_argument("--target-tokens", type=int, default=20_000 if action == "preview" else 1_000_000)
    audit_parser_v2 = corpus_actions.add_parser("audit", help="Summarize the audit ledger generated by preview or freeze")
    audit_parser_v2.add_argument("--output-dir", default="data/corpus_v2/preview")
    ...
    if args.command == "corpus":
        if args.corpus_action == "audit":
            records = load_audit(args.output_dir)
            by_decision: dict[str, int] = {}
            for record in records:
                decision = str(record.get("decision", "unknown"))
                by_decision[decision] = by_decision.get(decision, 0) + 1
            print(json.dumps({"audit_path": str(Path(args.output_dir) / "audit.jsonl"), "decisions": by_decision}, indent=2))
            return
        if args.corpus_root:
            os.environ["ARCHITECT_DATA_DIR"] = args.corpus_root
